...
class MultiTargetInferenceEngine:
    ...
    def __init__(self, 
        ...
        self.config.update({
            ...
            "shuffle_k_size": 8,  # Number of top recommendations to shuffle
            "shuffle_weight_exponent": 2.0,  # Controls how much to favor higher scores
    ...
    def _apply_discovery_shuffling(self, recommendations: List[MovieRecommendation], 
        ...
        """Apply discovery shuffling to top-k recommendations."""
        import random
        
        if len(recommendations) < self.config.get("min_recommendations_for_shuffle", 3):
            logger.info(f"Skipping shuffle - only {len(recommendations)} recommendations available")
            return recommendations
        
        shuffle_method = self.config.get("shuffle_method", "weighted")
        k = min(
            self.config.get("shuffle_k_size", 8), 
            len(recommendations)
        )
        
        if shuffle_method == "weighted":
            shuffled_recommendations = self._weighted_shuffle_top_k(recommendations, k)
            logger.info(f"Applied weighted shuffle to top-{k} recommendations for {session_key}")
        else:
            shuffled_recommendations = self._simple_shuffle_top_k(recommendations, k)
            logger.info(f"Applied simple shuffle to top-{k} recommendations for {session_key}")
    ...
    def _weighted_shuffle_top_k(self, recommendations: List[MovieRecommendation], 
                            k: int) -> List[MovieRecommendation]:
        """Weighted shuffle that favors higher scores but introduces controlled randomness."""
        import random
        ...
        for rec in top_k:
            # Convert score to weight (exponential to favor higher scores)
        ...
        while available_recs:
            try:
                ...
                # Fallback: just add remaining items in order
