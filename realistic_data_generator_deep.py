...
class ContentCatalog:
    ...
    # FIXED: Added the missing get_random_item method
...
class PersonaAgent:
    ...
    def execute_step(self, env: FireTVEnvironment) -> tuple[InteractionType, Dict]:
        ...
        self.think(env)
        if not self.state.plan:
            return InteractionType.HOVER, {'time_delta_seconds': 1.5}
...
class DeepRealisticDataGenerator:
    ...
    def __init__(self, config: SimulationConfig):
        self.config = config; self.fake = Faker()
        self.interactions = []; self.personas = []
        self.catalog = ContentCatalog(config.NUM_CONTENT_ITEMS, self.fake)
        print("🚀 Production-Scale Deep Simulation Data Generator Initialized.")
    ...
    def generate(self):
        for user_idx in tqdm(range(self.config.NUM_USERS), desc="Simulating Users"):
            ...
            for session_idx in range(max(1, num_sessions)):
                self._simulate_session(agent, session_idx)
        self._save_data()

    def _simulate_session(self, agent: PersonaAgent, session_idx: int):
        ...
        self._log_interaction(agent, session_id, env, InteractionType.SESSION_START, session_state, {})
        for _ in range(self.config.MAX_INTERACTIONS_PER_SESSION):
            ...
            self._log_interaction(agent, session_id, env, action, session_state, details)
            if action == InteractionType.HOME or agent.state.current_goal == GoalType.END_SESSION:
                break
        self._log_interaction(agent, session_id, env, InteractionType.SESSION_END, session_state, {})

    def _log_interaction(self, agent: PersonaAgent, session_id: str, env: FireTVEnvironment, action: InteractionType, session_state: Dict, details: Dict):
    ...
    def _save_data(self):
        ...
        pd.DataFrame(self.interactions).to_csv(self.config.OUTPUT_FILE, index=False)
        print(f"Production-scale dataset saved to {self.config.OUTPUT_FILE}")
